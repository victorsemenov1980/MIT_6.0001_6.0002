...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if pos2 in self.tile:
            if self.tile[pos2]>=0:
                self.tile[pos2]-=self.capacity
                return self.tile[pos2]
            else:
                return 0
        raise NotImplementedError
    ...
    def get_num_tiles(self):
        ...
        # return 'Number of tiles in the room='+str(self.width*self.height)
        raise NotImplementedError 
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        x=(round(pos[0]), round(pos[1]))
        if x not in self.furniture_tiles and x in self.tile:
            return True
        else:
            return False
        raise NotImplementedError
    ...
    def get_random_position(self):
        ...
        y=z2[1]
        z3=(x,y)
        if z3 not in self.furniture_tiles:
            return Position(x, y)
        raise NotImplementedError
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        while self.room.get_num_cleaned_tiles==0:
            if self.room.is_position_valid(self.position()) is True:
