...
with open('mit_map.txt','r') as file:
    for line in file:
        ...
        if int(x[0]) not in maps:
            maps[int(x[0])]=[]
        maps[int(x[0])].append(WeightedEdge(int(x[0]), int(x[1]), int(x[2]), int(x[3].rstrip())))
        
graph = Graph(maps)
y=graph.vertices()
for j in range(len(y)):
    x=graph.get_edges_for_node(y[j])
    for i in range(len(x)):
        print(WeightedEdge.get_source(x[i]),'-->',WeightedEdge.get_destination(x[i]),' Totoal distance=',WeightedEdge.get_total_distance(x[i]),' Distance outdoors=',WeightedEdge.get_outdoor_distance(x[i]))

# def find_best_path(graph, start_vertex, end_vertex, path=[]):
#         """ find all paths from start_vertex to 
#             end_vertex in graph """
#         shortest=None
#         path = path + [start_vertex]
...
#         if start_vertex == end_vertex:
#             return [path]
#         # if start_vertex not in x:
#         #     return []
#         for vertex in graph.get_edges_for_node(start_vertex):
#             if vertex not in path:
#                 if shortest == None or len(path) < len(shortest):
#                     extended_paths = find_best_path(vertex,end_vertex,path)                                   
#                     if extended_paths!=None:
#                         shortest=extended_paths
#         return shortest

# find_best_path(graph, 32, 35)
