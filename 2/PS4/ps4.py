...
def simulation_with_antibiotic(num_bacteria,
    ...
    resistant_pop = []
    for tri in range(num_trials):
        populations.append([None]*numStep)#create empty arrays
        resistant_pop.append([None]*numStep)
        bacList = []#list of bacteria in patient
        for bac in range(num_bacteria):#num bacterua we get as an argument
            bacList.append(ResistantBacteria(birth_prob, death_prob, resistant, mut_prob))
        pa = TreatedPatient(bacList, max_pop)#we get max pop as an argument and created the backlist
        for step in range(numStep):
            if step >= 150:
                pa.set_on_antibiotic()
                populations[tri][step] = pa.update()#we update the return array for total
                resistant_pop[tri][step] = pa.get_resist_pop() #we update the return array for resistant
            else:
                populations[tri][step] = pa.update()#same withou antibiotics for the first 150 steps
